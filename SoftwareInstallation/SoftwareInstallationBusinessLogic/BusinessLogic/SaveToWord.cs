...
namespace SoftwareInstallationBusinessLogic.BusinessLogic
{
    ...
    {
        //Создание документа с пакетами
        public static void CreateDocWithPackages(WordPackagesInfo info)
        {
        ...
        //Создание документа со складами
        public static void CreateDocWithWarehouses(WordWarehousesInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                MainDocumentPart mainPart = wordDocument.AddMainDocumentPart();
                ...
                foreach (var warehouse in info.Warehouses)
                {
                    ...
                    cellPerson.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellPerson.Append(new Paragraph(new Run(new Text(warehouse.WarehouseManagerFullName))));
