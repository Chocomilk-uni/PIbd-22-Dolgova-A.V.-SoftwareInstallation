...
namespace SoftwareInstallationBusinessLogic.BusinessLogic
{
    class SaveToExcel
    {
        public static void CreateDocWithPackageComponents(ExcelPackageComponentsInfo info)
        {
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                //Создаём SharedStringTable, если его нет
                if (sharedStringPart.SharedStringTable == null)
                {
                    sharedStringPart.SharedStringTable = new SharedStringTable();
                }

                //Создаём лист в книгу
        ...
        public static void CreateDocWithWarehouseComponents(ExcelWarehouseComponentsInfo info)
        {
                 ...
                 SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var warehouseComponent in info.WarehouseComponents)
                {
                    ...
                    foreach (var component in warehouseComponent.WarehouseComponents)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
        ...
        private static void InsertCellInWorksheet(ExcelCellParameters cellParameters)
        {
            ...
            {
                /*Все ячейки должны быть расположено последовательной друг за другом, поэтому
            ...
            //Вставляем текст
            cellParameters.SharedStringPart.SharedStringTable.AppendChild(new SharedStringItem(new Text(cellParameters.Text)));
            cellParameters.SharedStringPart.SharedStringTable.Save();

            cell.CellValue = new CellValue((cellParameters.SharedStringPart.SharedStringTable.Elements<SharedStringItem>().Count() - 1).ToString());
